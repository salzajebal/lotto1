...
export function PortalHome({
  reviews, posts, reviewsLoading, reviewsError, postsLoading, postsError, onModal, renderKakao, badgeSrc,
}: {
  reviews: Review[];
  posts: Post[];
  ...
  return <div className="portal-home">
    ...
    <section className="shell portal-membership" id="membership"><div><p className="portal-eyebrow gold">GOLDEN PICK MEMBERSHIP</p><h2>이번 주 분석 기준을<br /><em>회원 전용으로 받아보세요.</em></h2><p>주간 분석 번호 · 리포트 · 결과 복기 · 회원 커뮤니티</p></div><div className="portal-price"><small>3등 멤버십 / 1개월</small><strong>330,000<em>원</em></strong><button data-testid="button-membership-consult" onClick={() => onModal('membership')}>가입 상담 <ArrowRight size={15} /></button></div></section>
    <section className="shell portal-support" id="support"><div><p className="portal-eyebrow">CUSTOMER CENTER</p><h2>궁금한 점은<br /><em>카카오톡으로 물어보세요.</em></h2><p>1·2등 분석 번호는 회원님의 목표에 맞춘 별도 상담으로 안내합니다.</p></div>{renderKakao('portal-support-button')}<div className="portal-badge"><img src={badgeSrc} alt="한국고객만족도 1위 배지" /><span>고객만족도 및 신뢰 정보</span></div></section>
    ...
    <footer className="portal-footer"><div className="shell"><div><b>GOLDEN PICK</b><span>기록으로 확인하는 로또 분석 서비스</span></div><div><Link href="/reviews">당첨 증빙</Link><a href="#method">분석 기준</a><a href="#membership">멤버십</a><Link href="/community">커뮤니티</Link></div><small>© 2025 GOLDEN PICK · 본 페이지의 번호는 분석 예시이며 당첨을 보장하지 않습니다.</small></div></footer>
  </div>;
}