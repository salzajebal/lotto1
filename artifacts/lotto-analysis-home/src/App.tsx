...
function Home() {
  ...
  return (
    <div className="lotto-app" ref={revealRoot}>
      ...
      {modal === 'auth' && <Modal title={authMode === 'login' ? '다시 만나서 반갑습니다' : '로또리코 시작하기'} description={authMode === 'login' ? '분석 리포트와 커뮤니티를 이어서 확인하세요.' : '가입 신청 후 관리자 승인 절차를 거쳐 안내드립니다.'} onClose={() => setModal(null)}><div className="auth-switch"><button className={authMode === 'login' ? 'active' : ''} onClick={() => setAuthMode('login')}>로그인</button><button className={authMode === 'join' ? 'active' : ''} onClick={() => setAuthMode('join')}>회원가입</button></div>{submitted ? <div className="success-note">{authMode === 'login' ? '로그인 준비가 완료되었습니다. 곧 멤버 공간으로 이동합니다.' : '가입 신청이 접수되었습니다. 관리자 승인 후 안내드리겠습니다.'}</div> : <form className="form" onSubmit={submitForm}>{authMode === 'join' && <label>이름<input name="name" required placeholder="이름을 입력하세요" /></label>}<label>이메일<input name="email" required type="email" placeholder="name@example.com" /></label>{authMode === 'login' && <label>비밀번호<input name="password" required type="password" placeholder="6자 이상 입력하세요" /></label>}{submitError && <div className="form-error">{submitError}</div>}<button className="gold-button" type="submit" disabled={submitting}>{submitting ? '신청 중...' : authMode === 'login' ? '로그인하기' : '회원가입 신청하기'} <ArrowRight size={15} /></button></form>}</Modal>}
